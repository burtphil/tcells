...
thn_th1_cells = [get_cells(*simu)[:] for simu in simulation]
...
ax.set_ylim(0,3)
plt.tight_layout()
#fig.savefig("unlinked_prolif_death.pdf", bbox_inches = "tight")

#cells = [np.sum(th_gens[i], axis = 0) for i in range(nsim)]
#cells = np.asarray(cells)
#mean_cells = np.mean(cells, axis = 0)

#fig, ax = plt.subplots()
  
#ax.plot(simulation_time, mean_cells)
